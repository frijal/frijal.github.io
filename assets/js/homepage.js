// scripts/articles.js
// Client-side loader + renderer untuk artikel.json
...
let FILTERED = [];
let CURRENT_PAGE = 1;
let CURRENT_SORT = { field: 'date', dir: 'desc' }; // default sort newest first
...
export async function loadArticles() {
  try {
    ...
    // Expect data to be an array of articles with fields:
    // title, slug, url, image, datetime (ISO), category
    ...
    console.error(err);
    const el = qs('#main');
    if (el) el.innerHTML = `<div class="error">Terjadi kesalahan saat memuat data: ${err.message}</div>`;
  }
}
...
export function renderArticlesPage(page = 1) {
  CURRENT_PAGE = page;
...
/* Sidebar thumbnails (small list) */
export function renderSidebarThumbnails() {
...
/* Archive per tahun/bulan */
let ARCHIVE = {}; // { year: { month: [articles] } }
export function buildArchiveAndFilters() {
  ARCHIVE = {};
...
/* Filters UI (category/year/month selects + search) */
...
/* Populate month select when year chosen */
...
/* Apply cascading filter: accepts partial object {category, year, month, q} */
export function applyCascadingFilter({ category=null, year=null, month=null, q=null } = {}) {
  // read current selects if not provided
